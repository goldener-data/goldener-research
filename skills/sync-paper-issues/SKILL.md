...
description: Pull GitHub issues labeled "paper" from goldener-data/goldener-research
  ...
  branch, commit, push, open a PR adding each confirmed paper to the matching
  topic `BIBLIOGRAPHY.md` file(s), and close any source issues — fully
...
entries, branch, commit, push, open the PR, close any source issues, and
restore the working tree in one pass. Don't ask the user to approve the plan,
the research findings, the file placement, or the push/PR/close steps; just do
it and report what happened at the end. Only interrupt the run for a genuine
blocker you cannot resolve yourself (see "Blockers" below), or for a link
whose trustworthiness is genuinely ambiguous (step 5) — never to check in
on a step that succeeded. **Step 2 (prepare) and step 13 (restore) always
run, including on every blocker path** — this skill must never leave the repo
on an unexpected branch or with a dangling stash, whatever else happens in
between.

## Prerequisite: GitHub write access

Check this **before step 2** — before touching git at all. Step 11 (open PR)
and step 12 (close/comment on issues) need authenticated write access to
...
- If the `gh` CLI is installed, run `gh auth status`. A report of being logged in
  means write access is available; use `gh` for steps 11 and 12.
- Otherwise, check whether `$GITHUB_TOKEN` is set and non-empty in the environment.
  If so, use the REST API with that token for steps 11 and 12.
...
2. **Prepare the working tree.** Do this before anything else touches git.
   - Record the current branch: `git rev-parse --abbrev-ref HEAD`. This is the
     *initial branch* you must return to in step 13 — remember it verbatim.
   - Run `git status --porcelain`. If it reports anything (staged, unstaged, or
     ...
     Note that a stash was created — step 13 needs to know whether to pop one. If
     the tree is already clean, note that no stash was needed.
   - `git checkout main && git pull` so every later read of `BIBLIOGRAPHY.md`
     content (step 6's duplicate check) reflects current `main`.
...
3. **Build the worklist.** Each item is `{title, link, hint, source_issue}`,
   ...
   items for issue-sourced ones, since step 12 closes issues, not individual
   papers.
...
4. **Check for worklist items already covered by an open pull request.** Do
   ...
   handles — e.g. an earlier run opened the PR but its step 12 close failed
   ...
   - **Match found → in-progress.** Record the paper (and the issue number, if
     ...
     it further (skip steps 5–9 for it), and do not touch it in step 12.
...
5. **Resolve and confirm each candidate paper.** For each remaining worklist
   ...
   - **Before fetching any link — whether it came verbatim from the
     ...
     - **Genuinely ambiguous** (a domain you don't recognize but that could
       ...
       - **Stop the current task** — halt this skill run entirely rather than
         continuing past this item. Still run step 13 (restore the working
         tree) before ending, exactly like any other blocker; report which
         items were already handled before the stop, per "Notes".

       This is the one point in this skill where a genuine judgment call
       about safety is handed to the user instead of made autonomously — see
       the "runs end-to-end without stopping for confirmation" note above,
       which this narrowly overrides.
   - **If `link` is present and trusted per above**, fetch that page. Extract
     ...
     mangled or guessed title), first author, venue/year, and abstract. If the
     fetch fails or is blocked, fall through to the title-search case below
     (using whatever `title` is available).
   ...
   - **Confirm relevance from the abstract**: the abstract must plausibly
     ...
     - **Confirmed relevant** → proceed to step 6 for this item.
...
9. **Format and write the entry.** This format is identical, byte-for-byte,
   ...
   - `👤 First author:` is always exactly one `<First Last>` name — never
     `Authors:`, never `et al.`, never a full author list.
...
10. **Branch, commit, push — no confirmation.** By this point `main` is checked
    ...
    touching the existing one. Stage only the modified `BIBLIOGRAPHY.md` files —
    never `git add -A`. Commit message:
...
11. **Open the PR immediately — no confirmation.** Title: reuse the exact commit
    ...
    "direct request" if none), and why each is relevant (one sentence per
    paper, drawn from the abstract check in step 5) — reviewers should be able
    to see the reasoning without re-doing the research. Add a short section
    listing duplicates (step 6: paper title, file where already present),
    not-relevant items (step 5: paper title, what was checked), and
    in-progress items (step 4: paper, issue number if any, covering PR). Do not
    rely on `Closes #<n>` for auto-close — step 12 closes issues explicitly,
    and only covers items that actually have one.
...
12. **Close every issue-sourced item that was handled, each with an
    ...
    visible in the PR description (step 11) and the final report. An issue can
    have more than one paper (step 3), so resolve its outcome by looking at
    *all* of its worklist items together:
...
13. **Return to the initial branch and restore the working tree.** Last thing
...
- **No GitHub write access** (checked before step 2 — see "Prerequisite" above):
  ...
  there is nothing to restore and step 13 does not run either.
- **The worklist ends up empty** (no open `paper` issues found and no paper was
  ...
  create an empty branch/PR, jump to step 13. Still close the fully-resolved
  issue-sourced items (step 12) even with no PR — reaching either conclusion
  ...
  any unresolved item still on it.
- **The user chooses "stop the current task"** for an ambiguous link (step 5):
  stop the run right there. If a branch was already created and pushed with
  some entries committed, leave it as-is (don't roll it back) and report the
  branch name and what it contains so far; if nothing was committed yet,
  there's nothing to undo. Skip steps 10–12 for anything not yet reached, then
  still run step 13. Report exactly which worklist items were resolved
...
- **A candidate paper can't be identified at all** (step 5: no working link, and
  ...
  issue-sourced item, don't close the issue (see step 12's unresolved case) —
  ...
  and the same title/link suggestion.
- **PR creation fails** (e.g. a fine-grained PAT scoped to this org can return
  ...
  drafted description, skip step 12, then still run step 13.
- **`git push` fails**: report the exact error and the local branch name; don't
  retry destructive workarounds, skip step 12, then still run step 13.
- **Closing/commenting on an issue fails** (step 12): not a full-run blocker —
  keep going with the remaining issues, note the failures in the final report,
  still run step 13.

## Notes

- Never fabricate a paper's title, author, venue, abstract, or a relevance
  judgment — every claim in a `BIBLIOGRAPHY.md` entry must trace back to
  something you actually fetched and read (the paper's own page, the abstract).
  If a source is thin or ambiguous, say so in the PR description rather than
  guessing.
- A paper's *title* alone is never sufficient justification for relevance —
  step 5 requires the abstract to actually be read and to genuinely support
  relevance, not just share keywords with a repo theme.
- One issue (or one direct request) can legitimately produce zero, one, or
  several `BIBLIOGRAPHY.md` entries across one or more files — this is
  expected, not a bug to avoid. Track outcomes per paper (step 3's worklist
  items), but close/comment per issue (step 12), since that's the unit GitHub
  exposes; a direct-request item has no issue to close.
...
- Never reuse a pre-existing branch for this run's commits, even one that looks
  unfinished (pushed, no PR, matches the naming pattern). Step 10 always creates
...
- Being autonomous means not pausing between successful steps — it does not mean
  ...
  (issue number if any, paper, why identification failed), files changed,
  branch name, the PR URL (or the blocker reached instead), the outcome of
  closing each issue-sourced item in step 12, and the step 13 outcome.
